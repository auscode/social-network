...
def save_post(request):
    if request.method == "POST":
        ...
        return JsonResponse(
            {"error": f"request methods supported : POST, GET"}, status=400
        )
...
def update_like(request, post_id):
    ...
    if post in profile.get_all_liked_post.all():
        newStatus = False
    ...
    post.save()
    return JsonResponse(
        {"liked": newStatus, "newAmount": post.likes.count()}, status=200
    )
...
def update_follow(request, profile_id):
    ...
    profile.save()
    return JsonResponse(
        {"newFollower": newStatus, "newAmount": profile.followers.count()}, status=200
    )
