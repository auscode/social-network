document.addEventListener('DOMContentLoaded', function() {
    ...
    window.history.pushState('The Network','The Network','https://127.0.0.1:8000/');
...
function show_profile(creator_id) {
    ...
    .then (profile => {
        ...
        if (profile.follow_available) {
            ...
            follow_buttton.addEventListener("click",() => update_follow(creator_id));
        }
    })
    window.scrollTo(0,0);
}
...
function build_post(post) {
    ...
    if (post.editable){
        ...
        edit.innerHTML = "Edit";
        edit.addEventListener("click",() => edit_post(psot));
...
function update_follow(profile_id){
    fetch(`/profiles/${profile_id}/update_follow`)
    .then (response => response.json())
    .then(response => {
        follow_button = document.getElementById("follow-button");
...
function update_like(post){
    fetch(`/posts/${post.id}/update_like`)
    .then (response => response.json())
    .then (response => {
        if (response.liked){
            document.getElementById(`like-icon-${post.id}`).className ="icon-heart col-auto";
        }else{
            doocument.getElementById(`like-icon-${post.id}`).className = "icon-heart-empty col-auto";
        }
...
function edit_post(post) {
    ...
    edit_buttons_row.appendChild(content_editable);
    ...
    edit_buttons_row.appendChild(cancel_button);
